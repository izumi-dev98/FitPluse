...
export default function DashboardPage() {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            ...
            {goalType && <> · <span className="capitalize text-slate-300">{goalType}</span></>}
          </p>
        </div>
        <button
      ...
      {loading ? (
        <Card className="text-center py-12 text-slate-400">Loading your day…</Card>
      ) : (
        <>
          <div className="grid lg:grid-cols-3 gap-4 mb-4">
            <Card className="lg:col-span-2 flex flex-col sm:flex-row items-center gap-6">
              <Ring percent={pct}>
                <div className="text-3xl font-extrabold text-white">{Math.round(consumed)}</div>
              ...
              <div className="flex-1 w-full space-y-3">
                ...
                {!goalCalories && (
                  <button onClick={() => navigate('/goals')} className="text-sm text-brand-400 font-semibold">
                    Set a calorie goal →
                  </button>
                )}
              </div>
            ...
            <Card>
              ...
              <div className="space-y-4">
                <MacroRow label="Protein" current={protein} target={proteinTarget} color="text-protein" bar="bg-protein" />
                <MacroRow label="Carbs" current={carbs} target={carbTarget} color="text-carbs" bar="bg-carbs" />
                <MacroRow label="Fat" current={fat} target={fatTarget} color="text-fat" bar="bg-fat" />
              </div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
            ...
            <Card>
              <div className="flex items-center gap-2 text-brand-400 text-xs font-semibold mb-2"><Footprints size={16} /> Steps</div>
              <div className="text-2xl font-extrabold text-white">{steps.toLocaleString()}</div>
              <div className="mt-2 h-1.5 bg-slate-800 rounded-full overflow-hidden">
            ...
            <Card>
              <div className="flex items-center gap-2 text-slate-300 text-xs font-semibold mb-2"><Scale size={16} /> Weight</div>
              <div className="text-2xl font-extrabold text-white">{latestWeight ?? '—'}{latestWeight != null && <span className="text-sm font-medium text-slate-500"> kg</span>}</div>
              <button onClick={() => navigate('/progress')} className="text-[11px] text-brand-400 font-semibold mt-2">Update →</button>
            </Card>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              >
                ...
                <div className="font-bold text-white group-hover:text-brand-400">{item.label}</div>
